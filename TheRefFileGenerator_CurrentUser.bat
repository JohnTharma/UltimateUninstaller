...
for /f "tokens=*" %%a in ('reg query "HKCU\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall" /s /f "%SOFTWARE_NAME%" /reg:32 2^>nul') do (
    set "KEY=%%a"
...
if not defined SOFTWARE_FOUND (
    for /f "tokens=*" %%a in ('reg query "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall" /s /f "%SOFTWARE_NAME%" /reg:64 2^>nul') do (
        set "KEY=%%a"
        for /f "tokens=1,*" %%b in ('reg query "!KEY!" /v DisplayName 2^>nul') do (
            if "%%b"=="DisplayName" (
                set "DISPLAY_NAME=%%c"
